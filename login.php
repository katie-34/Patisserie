...
if (isset($_POST['envoi'])) {
    ...
    $select_user = $conn->prepare("SELECT * FROM utilisateurs WHERE Email = ? AND mot_de_passe = ?");
    $select_user->execute([$email,$pass]);
    $row = $select_user->fetch(PDO::FETCH_ASSOC);


    if ($select_user->rowCount() > 0) {
       setcookie('id_utilisateur', $row['id'], time() + 60*60*24*30, '/');
...
<head>
    ...
    <title>Sweet Fresh Tasty Bakery - Formulaire utilisateur</title>
